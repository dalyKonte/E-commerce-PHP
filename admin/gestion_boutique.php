...
if($_POST)
{
    ...
    {
        // en cas de modification, si nous ne modifions pas la photo, nous récuperons la valeur du champ type 'hidden' donc l'URL de la photo actuelle afin de l'affecter à la variable $photoBdd donc de réinserer l'URL de l'image dans la Bdd
    ...
    // SI l'indice 'action' est bien defini dans l'URL et qu'il a pour valeur 'ajout', alors on execute une requete d'insertion à la validation du formulaire
    ...
    {
        // UPDATE BDD PRODUIT
        //Exo : realiser le traitement SQL+PHP permettant de modifier un produit en fonction de l'id_produit transmis dans l'URL
        ...
        $_GET['action'] = 'affichage';// on redirige vers l'affichage des produits apres la modification

        $v = "<p class='col-md-3 mx-auto bg-success text-center text-white p-3 rounded'>Le produit titre <strong>ID $_POST[titre]</strong>référence <strong>$_POST[reference]</strong> a bien été modifié !</p>";
    }
...
if(isset($_GET['action']) && $_GET['action'] == 'affichage') 
{
    ...
    while ($p = $r->fetch(PDO::FETCH_ASSOC)) {
        ...
        foreach ($p as $k => $v) {
            if ($k == 'photo') {
                echo "<td><img src='$v' alt='' style='width: 50px;'></td>";
            } else {
...
?>
    ...
    <form method="post" class="col-md-6 mx-auto" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <!-- Un champ type file ne peut pas avoir d'attribut value affecter, c'est pourquoi nous definissons un champ de type hidden ci dessous afin de recuperer l'URL de l'image en cas de modification -->
        </div>
        <!--On déclare un champ type hidden afin de récuperer l'URL de l'image pour la renvoyer dans la BDD si l'internaute en cas de modification ne souhaite pas modifier l'image  -->
